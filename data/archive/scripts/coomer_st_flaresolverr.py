#!/usr/bin/env python3
"""
Enhanced Coomer.st Scraper with FlareSolverr Integration
-----------------------------------------------------

A crawler for coomer.st that bypasses Cloudflare protection using FlareSolverr.
This tool scrapes:
...
Usage:
    python coomer_st_flaresolverr.py --creator <creator_id> --service onlyfans
"""
...
class FlareSolverrCoomerScraper:
    ...
    def get_with_fallback(self, url: str, use_flaresolverr: bool = True):
        ...
        """
        # For now, always use FlareSolverr since it works
        if use_flaresolverr:
    ...
    def download_media_file(self, media_url: str, local_path: str) -> bool:
        ...
        # Use FlareSolverr for media downloads too
        try:
    ...
    def scrape_creator(self, creator_id: str, service: str = "onlyfans", max_posts: int = 50):
        ...
        if profile:
            ...
            print(f"👤 Found creator: {creator_id}")
        ...
        for i, post in enumerate(posts):
            print(f"📸 Post {i+1}/{len(posts)}: {post.get('title', 'No title')}")
