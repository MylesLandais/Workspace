...
## Cloudflare-Protected Archives Issue

### Problem
Archived.moe and similar sites use Cloudflare anti-bot protection that blocks automated requests:
- Direct HTTP requests return 403/405 errors
- Requires JavaScript and browser-like headers
- Blocks crawlers/archiving tools

### Self-Hosted Solutions to Investigate

1. **FlareSolverr**
   - Docker-based proxy that bypasses Cloudflare
   - Acts as a headless browser proxy
   - Deploy alongside crawler infrastructure

2. **Cloudflare-Scraper**
   - Python library for Cloudflare bypass
   - May need additional self-hosted proxy

3. **Playwright/Puppeteer with Stealth**
   - Headless browser approach
   - Stealth plugins to avoid detection

### Implementation Tasks
- [ ] Evaluate FlareSolverr for archived.moe
- [ ] Add Cloudflare proxy service to docker-compose.yml
- [ ] Update crawler code to use Cloudflare proxy for protected sites
- [ ] Document proxy configuration in crawler setup
