...
export class ImageHasher {
  async computeHashes(imagePath: string): Promise<ImageHashes> {
    ...
    const phash =
      typeof phashStr === "string" ? BigInt("0x" + phashStr) : BigInt(phashStr);
    const dhash =
      typeof dhashStr === "string" ? BigInt("0x" + dhashStr) : BigInt(dhashStr);
  ...
  async computeHashesFromBuffer(imageBuffer: Buffer): Promise<ImageHashes> {
    ...
    try {
      ...
      const phash =
        typeof phashStr === "string"
          ? BigInt("0x" + phashStr)
          : BigInt(phashStr);
      const dhash =
        typeof dhashStr === "string"
          ? BigInt("0x" + dhashStr)
          : BigInt(dhashStr);
