...
export async function addTagsToSource(
  id: string,
  tags: string[],
): Promise<string[]> {
  const session = getSession();
  try {
    const query = `
      MATCH (s:Source {id: $id})
      SET s.tags = [x IN (coalesce(s.tags, []) + $tags) WHERE x IS NOT NULL | x]
      WITH s, [x IN s.tags | trim(x)] AS cleaned
      SET s.tags = apoc.coll.toSet(cleaned)
      RETURN s.tags AS tags
    `;

    const result = await session.run(query, { id, tags });
    return result.records[0]?.get("tags") || [];
  } finally {
    await session.close();
  }
}

export async function removeTagsFromSource(
  id: string,
  tags: string[],
): Promise<string[]> {
  const session = getSession();
  try {
    const query = `
      MATCH (s:Source {id: $id})
      SET s.tags = [t IN coalesce(s.tags, []) WHERE NOT t IN $tags]
      RETURN s.tags AS tags
    `;
...
export async function bulkAddTags(
  ...
  try {
    const query = `
      MATCH (s:Source)
      WHERE s.id IN $ids
      SET s.tags = [x IN (coalesce(s.tags, []) + $tags) WHERE x IS NOT NULL | trim(x)]
      WITH s, s.tags AS raw
      SET s.tags = apoc.coll.toSet(raw)
      RETURN count(s) AS updated
    `;

    const result = await session.run(query, { ids, tags });
