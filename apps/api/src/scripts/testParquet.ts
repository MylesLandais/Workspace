...
async function testRead() {
  const threadsPath =
    "/home/warby/Workspace/jupyter/packs/nightly-2026-01-02/threads.parquet";
  const imagesPath =
    "/home/warby/Workspace/jupyter/packs/nightly-2026-01-02/images.parquet";
  ...
  await parquetRead({
    file: {
      ...
      async slice(offset, end) {
        return threadsBuffer.buffer.slice(
          threadsBuffer.byteOffset + offset,
          threadsBuffer.byteOffset + end,
        );
      },
    },
    rowFormat: "object",
  ...
  await parquetRead({
    file: {
      ...
      async slice(offset, end) {
        return imagesBuffer.buffer.slice(
          imagesBuffer.byteOffset + offset,
          imagesBuffer.byteOffset + end,
        );
      },
    },
    rowFormat: "object",
