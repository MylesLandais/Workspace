...
export const mutationResolvers: Resolvers = {
  Mutation: {
    ...
    // Tag mutations
    addTagsToSource: async (_, { id, tags }) => {
      return await addTagsToSource(id, tags);
    },

    removeTagsFromSource: async (_, { id, tags }) => {
      return await removeTagsFromSource(id, tags);
    },

    setSourceTags: async (_, { id, tags }) => {
      return await setSourceTags(id, tags);
    },

    bulkAddTags: async (_, { ids, tags }) => {
      return await bulkAddTags(ids, tags);
    },

    bulkRemoveTags: async (_, { ids, tags }) => {
      return await bulkRemoveTags(ids, tags);
    },
