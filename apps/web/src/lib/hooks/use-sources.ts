...
export function useSourceMutations(): UseSourceMutationsReturn {
  ...
  const addTags = useCallback(
    async (id: string, tags: string[]): Promise<string[]> => {
      const mutation = `
      mutation AddTagsToSource($id: ID!, $tags: [String!]!) {
        addTagsToSource(id: $id, tags: $tags)
      }
    `;

      const result = await executeMutation<{ addTagsToSource: string[] }>(
        mutation,
        { id, tags },
      );
      return result.addTagsToSource;
    },
    [executeMutation],
  );
  ...
  const removeTags = useCallback(
    async (id: string, tags: string[]): Promise<string[]> => {
      const mutation = `
      mutation RemoveTagsFromSource($id: ID!, $tags: [String!]!) {
        removeTagsFromSource(id: $id, tags: $tags)
      }
    `;

      const result = await executeMutation<{ removeTagsFromSource: string[] }>(
        mutation,
        { id, tags },
      );
      return result.removeTagsFromSource;
    },
    [executeMutation],
  );
  ...
  const setTags = useCallback(
    async (id: string, tags: string[]): Promise<string[]> => {
      const mutation = `
      mutation SetSourceTags($id: ID!, $tags: [String!]!) {
        setSourceTags(id: $id, tags: $tags)
      }
    `;

      const result = await executeMutation<{ setSourceTags: string[] }>(
        mutation,
        { id, tags },
      );
      return result.setSourceTags;
    },
    [executeMutation],
  );

  const bulkAddTags = useCallback(
    async (ids: string[], tags: string[]): Promise<number> => {
      const mutation = `
      mutation BulkAddTags($ids: [ID!]!, $tags: [String!]!) {
        bulkAddTags(ids: $ids, tags: $tags)
      }
    `;

      const result = await executeMutation<{ bulkAddTags: number }>(mutation, {
        ids,
        tags,
      });
      return result.bulkAddTags;
    },
    [executeMutation],
  );
  ...
  const bulkRemoveTags = useCallback(
    async (ids: string[], tags: string[]): Promise<number> => {
      const mutation = `
      mutation BulkRemoveTags($ids: [ID!]!, $tags: [String!]!) {
        bulkRemoveTags(ids: $ids, tags: $tags)
      }
    `;

      const result = await executeMutation<{ bulkRemoveTags: number }>(
        mutation,
        { ids, tags },
      );
      return result.bulkRemoveTags;
    },
    [executeMutation],
  );
...
export function useAllTags(): {
  ...
  const fetchTags = useCallback(async () => {
    ...
    try {
      ...
      setTags(result.data.getAllTags || []);
    } catch (err) {
      setError((err as Error).message);
      setTags([]);
    } finally {
