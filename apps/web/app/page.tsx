 ...
 * - Server Component by default (hero content paints immediately)
 * - Auth redirect in small client island (non-blocking)
 * - Waitlist form lazy-loaded (reduces initial bundle)
 *
 * Features:
...
// Lazy load the form - it's interactive and below the fold
// This reduces the initial JS bundle by ~15-20KB
const WaitlistForm = dynamic(
  () =>
    import("@/components/waitlist/WaitlistForm").then((mod) => ({
      default: mod.WaitlistForm,
    })),
  {
    loading: () => (
      <div className="w-full max-w-md h-24 bg-zinc-800/20 border border-white/10 rounded-lg animate-pulse" />
    ),
  },
);

export default function Home() {
  // Server Component - renders immediately with static HTML
  // The hero content (LCP) paints at ~0.9s instead of waiting for JS

  return (
    <>
      {/* Non-blocking auth redirect - runs in background after page paints */}
      <AuthRedirect />

      <main className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center p-4 relative overflow-hidden">
        {/* Existing users sign in link */}
        <Link
          href="/auth"
        ...
        {/* Main content - LCP element (Hero section) */}
        <div className="w-full flex flex-col items-center gap-16 relative z-10">
          <div className="flex flex-col items-center space-y-6 text-center">
            ...
            {/* Hero heading - This is the LCP element */}
            ...
            <p className="text-zinc-500 max-w-md text-lg font-medium tracking-tight">
              Community pages coming soon. Join the mailing list to receive a
              beta invite.
            </p>
          </div>

          {/* Waitlist form - Lazy loaded (non-critical for LCP) */}
          <WaitlistForm />
