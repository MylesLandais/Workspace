...
- `client/static/index.html` - Character sheet frontend
...
### Docker Notes
- Workspace mounted at `/home/jovyan/workspaces` (plural) in container
...
- [x] Schema migrations (`database/migrations/001_initial_schema.cypher`)
...
- [x] Web frontend for character sheet viewing (`client/static/index.html`)
...
In `/home/jovyan/workspaces/.env` (Docker) or `~/Workspace/jupyter/.env` (local):
```
...
The workspace is mounted at `/home/jovyan/workspaces` (plural) in the Docker container as specified in `docker-compose.yml`.
...
```bash
docker exec -d jupyter bash -c "cd /home/jovyan/workspaces/rpg-graph-vtt/web && uvicorn server:app --host 0.0.0.0 --port 8000"
```
...
**Note:** The workspace is mounted at `/home/jovyan/workspaces` (plural) as specified in `docker-compose.yml`. Use this path in all Docker commands.
