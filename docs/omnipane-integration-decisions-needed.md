# OmniPane Integration - Decisions Needed

This document highlights key conflicts and decisions that need to be resolved before proceeding with OmniPane integration. Please review and provide guidance on each item.

## Critical Decisions

### 1. Platform Enum Extensions

**Conflict**: OmniPane supports source types not in Bunny's Platform enum.
...
**OmniPane Additional Types Needed**:
- RSS
- NEWSLETTER
- BOORU
- MONITOR
...
**Conflict**: OmniPane is single-user, Bunny needs multi-user support.
...
**Conflict**: OmniPane uses "Article", Bunny uses "Media" and "Post".
...
**Conflict**: OmniPane "Boards" are user collections of articles, Bunny "FeedGroups" are source collections.
...
**Conflict**: OmniPane uses tags extensively, Bunny doesn't have explicit tag system.
...
**Conflict**: OmniPane manages state in local storage, needs to be user-specific in multi-user system.
...
**Conflict**: OmniPane uses client-side AI, needs to be backend for security.
...
**Conflict**: OmniPane's Article model vs Bunny's Media/Post model.
...
**Conflict**: OmniPane uses React 19, Bunny uses React 18.

**Status**: Already decided for Lumina integration - upgrade to React 19.

**Action**: Proceed with React 19 upgrade (already in progress for Lumina).
