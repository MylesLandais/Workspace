# Reader Tech Stack Conflicts and Integration Points

## Overview

This document identifies conflicts between Reader Reader's tech stack and Bunny's existing architecture, and proposes integration strategies.

## Tech Stack Comparison

### Frontend

| Component | Reader | Bunny | Conflict Level |
|-----------|----------|-------|----------------|
...
**Conflict**: Reader uses React 19.2.3, Bunny uses React 18.3.1
...
**Resolution**:
- Upgrade Bunny frontend to React 19 (aligns with Bunny integration decision)
- OR adapt Reader components to React 18 (may lose some features)
- **Recommendation**: Upgrade to React 19 (already planned for Bunny)

### 2. State Management Architecture

**Conflict**: Reader uses LocalStorage + React state, Bunny uses GraphQL + Apollo Client

**Impact**:
- Reader components expect local state management
...
**Resolution**:
- Port Reader components to use Apollo Client hooks
...
**Conflict**: Reader stores data in LocalStorage (planned), Bunny uses Neo4j
...
**Resolution**:
- Map Reader data structures to Neo4j schema
...
**Conflict**: Reader supports more source types than Bunny
...
- Extend Platform enum to include:
  ...
  - `BOORU`
  - `MONITOR`
- Map Reader source types to Bunny platforms
...
**Conflict**: Reader "Boards" vs Bunny "FeedGroups"

**Impact**:
- Reader Boards = user collections of articles (Pinterest-style)
...
**Conflict**: Reader uses free-form tags, Bunny doesn't have explicit tag system

**Impact**:
- Tags are core to Reader's filtering/organization
...
**Conflict**: Reader uses Google Gemini client-side, Bunny plans backend AI agents

**Impact**:
- API key exposure in client-side code
- Generation happens on-demand in browser
- No caching or rate limiting in Reader approach
...
**Conflict**: Reader manages read/saved/archived in local state, needs user-specific persistence
...
**Conflict**: Reader aggregates articles from multiple sources, Bunny focuses on Reddit

**Impact**:
- Reader expects unified feed from multiple source types
...
**Conflict**: Reader has list/grid modes, Bunny currently has grid
...
1. Design GraphQL schema extensions:
   ...
   - AI summary storage
2. Map Reader data models to Neo4j schema
...
1. Upgrade React to v19 (if not already done for Bunny)
2. Port Reader components to use Apollo Client:
...
### Medium Risk
- **React Version**: Upgrade to React 19 may introduce breaking changes (mitigated by Bunny decision)
...
3. **Preserve UX**: Keep Reader's excellent UI/UX patterns
...
5. Plan React 19 upgrade (if not already done for Bunny)
