# Reader Data Schema Expectations

This document defines the Neo4j schema requirements for integrating Reader Reader with Bunny's backend.

## Overview

Reader's data model centers around Articles, Boards, Sources, Tags, and User-specific state. This needs to be mapped to Neo4j graph structure while maintaining compatibility with Bunny's existing schema.
...
Reader's `Article` should map to Bunny's existing `Media` or `Post` nodes. Options:
...
Reader's `Source` maps to existing `Handle` or `Subreddit` nodes. Options:
