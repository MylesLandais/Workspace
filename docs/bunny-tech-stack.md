# Bunny Tech Stack Conflicts and Integration Points

## Overview

This document identifies conflicts between Bunny Feed's tech stack and Bunny's existing architecture, and proposes integration strategies.

## Tech Stack Comparison

### Frontend

| Component | Bunny | Bunny | Conflict Level |
|-----------|--------|-------|----------------|
...
**Conflict**: Bunny uses React 19, Bunny uses React 18
...
- OR adapt Bunny components to React 18 (may lose some features)
- **Recommendation**: Upgrade to React 19 for forward compatibility

### 2. State Management Architecture

**Conflict**: Bunny uses LocalStorage + React state, Bunny uses GraphQL + Apollo Client

**Impact**:
- Bunny components expect local state management
- Bunny architecture requires GraphQL queries/mutations
- Data synchronization patterns differ

**Resolution**:
- Port Bunny components to use Apollo Client hooks
...
**Conflict**: Bunny stores data in LocalStorage, Bunny uses Neo4j
...
**Conflict**: Bunny uses Google Gemini client-side, Bunny plans backend AI agents

**Impact**:
- API key exposure in client-side code
- Generation happens on-demand in browser
- No caching or rate limiting in Bunny approach
...
**Conflict**: Bunny generates feeds client-side, Bunny uses real ingestion pipeline

**Impact**:
- Bunny can generate on-demand mock content
...
**Conflict**: Bunny has dynamic `SourceLink[]`, Bunny has `Handle` nodes with relationships

**Impact**:
- Data structure similarity but implementation differs
- Bunny sources are part of identity object
...
**Conflict**: Bunny "Boards" are filter presets, Bunny "FeedGroups" are source collections
...
**Conflict**: Bunny uses CSS variables, Bunny theme system unknown
...
- Integrate Bunny theme variables
...
2. Map Bunny data models to Neo4j schema
...
2. Port Bunny components to use Apollo Client
...
3. **Preserve UX**: Keep Bunny's excellent UI/UX patterns
