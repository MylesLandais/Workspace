# Reader Reader Codebase Analysis

## Overview

Reader Reader is a unified content consumption platform that aggregates articles and media from multiple sources (RSS, Twitter, newsletters, YouTube, Reddit, booru sites, and monitoring services) into a single reading interface with AI-assisted features.
...
Reader's `Article` maps to Bunny's `Media`/`Post` nodes:
- `Article.id` → `Media.id`
...
Reader source types map to Bunny platforms:
...
Reader uses free-form tags. In Neo4j:
