...
export class UrlCacheService {
  ...
  async getCachedUrl(sha256: string, variant: UrlVariant): Promise<string | null> {
    const key = `presigned:${sha256}:${variant}`;
    return await this.client.get(key);
  }

  async setCachedUrl(sha256: string, variant: UrlVariant, url: string): Promise<void> {
    const key = `presigned:${sha256}:${variant}`;
    await this.client.setEx(key, this.ttl, url);
  }

  async batchGetUrls(sha256s: string[], variant: UrlVariant): Promise<Map<string, string | null>> {
    if (sha256s.length === 0) {
      return new Map();
    }
