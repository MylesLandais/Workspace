...
async function main() {
  ...
  try {
    const mediaResult = await session.run('MATCH (m:Media) RETURN count(m) as count');
    console.log('Media nodes:', mediaResult.records[0].get('count').toNumber());

    const clusterResult = await session.run('MATCH (c:ImageCluster) RETURN count(c) as count');
    console.log('Image clusters:', clusterResult.records[0].get('count').toNumber());

    const postResult = await session.run('MATCH (p:Post) RETURN count(p) as count');
