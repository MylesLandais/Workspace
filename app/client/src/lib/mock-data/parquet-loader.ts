...
async function loadParquetCache(): Promise<ParquetCache | null> {
  ...
  try {
    ...
    if (cachedData) {
      console.log(
        `Loaded parquet cache: ${cachedData.metadata.totalThreads} threads, ${cachedData.metadata.totalImages} images`,
      );
    }
    return cachedData;
  } catch (error) {
...
export async function generateParquetFeed(): Promise<FeedItem[]> {
  ...
  for (const thread of cacheData.threads.slice(0, 20)) {
    // Limit to 20 threads for performance
    ...
    const feedItem: FeedItem = {
      ...
      galleryUrls: images.map(
        (img) => `/imageboard/b/${threadId}/${img.filename}`,
      ),
