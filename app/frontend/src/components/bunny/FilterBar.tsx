...
import { FilterState } from '../../lib/bunny/types';
...
export const FilterBar: React.FC<FilterBarProps> = ({ filters, setFilters, onMenuClick, onRefresh, onSaveBoard }) => {
  ...
  const removeFilter = (type: 'person' | 'source' | 'tag', value: string) => {
    setFilters(prev => ({
      ...prev,
      ...
      tags: type === 'tag' ? prev.tags.filter(t => t !== value) : prev.tags,
    }));
  };

  const hasActiveFilters = filters.persons.length > 0 || filters.sources.length > 0 || filters.tags.length > 0 || filters.searchQuery.length > 0;
    ...
    <div className="sticky top-0 z-30 bg-app-bg/80 backdrop-blur-xl border-b border-app-border transition-colors duration-300">
      <div className="px-6 py-4">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 relative group">
              ...
              <input 
                ...
                placeholder="Ask Bunny... (e.g., 'New minimalist architecture shots')"
          ...
          {(filters.persons.length > 0 || filters.sources.length > 0 || filters.tags.length > 0) && (
            <div className="flex flex-wrap gap-2 animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              {filters.tags.map(tag => (
                <div key={tag} className="flex items-center gap-1 pl-3 pr-2 py-1 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs rounded-full">
                  <span>{tag}</span>
                  <button onClick={() => removeFilter('tag', tag)} className="p-0.5 hover:bg-emerald-500/20 rounded-full">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ))}
