/**
 * Bunny GUI: Entity Management View (Mock Data Version)
 * 
 * Person/Actor entity management using the Mock Data Factory
 * Provides a standardized way of managing frontend validations at scale
 */

import React, { useState } from 'react';
import type { IdentityProfile, Relationship, SourceLink, PlatformType } from '../../lib/bunny/types';
import { 
  mockDataFactory, 
  getIdentities, 
  getIdentityBySlug, 
...
export const EntityManagerView: React.FC<EntityManagerViewProps> = ({ onBack }) => {
  ...
  // Get identities from mock data factory
  const identities = getIdentities();

  const handleSelect = (id: string) => {
    const identity = getIdentityBySlug(id);
    if (identity) {
      setSelectedId(id);
  ...
  const handleSave = () => {
    if (formState && selectedId) {
      // Update the identity in mock factory
      mockDataFactory.identities = {
        ...identities,
  ...
  const handleDelete = (id: string) => {
    if (window.confirm(`Delete ${identities[id]?.name}?`)) {
      ...
      delete updatedIdentities[id];
      mockDataFactory.identities = updatedIdentities;
  ...
  const handleRemoveRelationship = (targetId: string) => {
    if (formState && selectedId) {
      const updatedRelationships = formState.relationships.filter(r => r.targetId !== targetId);
      const updatedProfile: IdentityProfile = {
        ...formState,
  ...
  const handleRemoveSource = (sourceIdOrIndex: string | number) => {
    if (formState && selectedId) {
      const sourceIndex = typeof sourceIdOrIndex === 'number' 
        ? sourceIdOrIndex
        : formState.sources.findIndex(s => s.id === sourceIdOrIndex || s.databaseId === sourceIdOrIndex);
      
      if (sourceIndex === -1) return;
      
      const updatedSources = formState.sources.filter((_, idx) => idx !== sourceIndex);
      const updatedProfile: IdentityProfile = {
        ...formState,
  ...
  const handleToggleSourceVisibility = (sourceIdOrIndex: string | number, currentHidden: boolean) => {
    if (formState && selectedId) {
      const sourceIndex = typeof sourceIdOrIndex === 'number' 
        ? sourceIdOrIndex
        : formState.sources.findIndex(s => s.id === sourceIdOrIndex || s.databaseId === sourceIdOrIndex);
      ...
      const source = formState.sources[sourceIndex];
      if (!source) return;
      
      const updatedSources = formState.sources.map((s, idx) => 
        idx === sourceIndex 
  ...
  const filteredIdentities = Object.values(identities).filter(p => 
    p.name.toLowerCase().includes(filterText.toLowerCase()) ||
    p.aliases.some(a => a.toLowerCase().includes(filterText.toLowerCase()))
  );
          ...
          <div className="p-6 md:p-10 max-w-4xl mx-auto space-y-8 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2 mb-4">
                    {formState.sources.map((src, idx) => (
                        ...
                        >
                            ...
                            {/* Toggle Visibility */}
                            <button 
                                onClick={() => handleToggleSourceVisibility(idx, src.hidden)}
              ...
              <div className="bg-app-surface border border-app-border rounded-xl p-6 space-y-4">
                ...
                <p className="text-xs text-app-muted mb-4">Keywords help AI find relevant content and generate accurate captions.</p>
