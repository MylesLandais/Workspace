...
const OUTPUT_DIR = '/home/warby/Workspace/Bunny/app/client/public/parquet-cache';
...
async function main() {
  try {
    ...
    const uniqueThreads = Array.from(new Set(threads.map(t => t.thread_id.toString())));
    console.log(`Found ${uniqueThreads.length} unique threads out of ${threads.length} total`);
    
    uniqueThreads.forEach((threadId, index) => {
      const imagesPerThread = Math.max(1, Math.floor(images.length / uniqueThreads.length));
      ...
      console.log(`Thread ${threadId}: assigned ${assignedImages.length} images`);
    });
    ...
    console.log(`Thread mapping has ${Object.keys(threadMappingObj).length} entries`);
    ...
    console.log(`  - ${Object.keys(threadMappingObj).length} thread-image mappings`);
