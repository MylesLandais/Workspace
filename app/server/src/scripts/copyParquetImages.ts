...
const OUTPUT_DIR = '/home/warby/Workspace/Bunny/app/client/public/parquet-images';
...
async function copyImagesToPublic() {
  try {
    ...
    for (const image of images.slice(0, 100)) { // Limit to first 100 images for testing
      const sourcePath = join(PACKS_DIR, `${LATEST_PACK}-full/images`, image.filename);
