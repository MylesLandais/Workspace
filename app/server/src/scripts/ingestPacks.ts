...
import { ImageIngestionService, IngestionMetadata } from '../services/imageIngestion.js';
...
async function ingestImageboardPack(packPath: string, ingestionService: ImageIngestionService, limit?: number) {
  ...
  console.log(`Found ${images.length} images (processing ${subset.length}) and ${threads.length} threads`);
  ...
  const fullPackPath = packPath.includes('-full') ? packPath : packPath.replace(/nightly-(\d{4}-\d{2}-\d{2})/, 'nightly-$1-full');
  ...
  for (let i = 0; i < subset.length; i++) {
    ...
    try {
      ...
      process.stdout.write(`  [${i+1}/${subset.length}] Ingesting ${img.filename}... `);
...
async function ingestRedditPack(packPath: string, ingestionService: ImageIngestionService, limit?: number) {
  ...
  const subset = limit ? images.slice(0, limit) : images;
  console.log(`Found ${images.length} images (processing ${subset.length}) and ${posts.length} posts`);
  ...
  for (let i = 0; i < subset.length; i++) {
    ...
    try {
      ...
      const fileName = img.image_url.split('/').pop()?.split('?')[0] || `${img.post_id}.jpg`;
      const localPath = join(localImagesDir, fileName);
      
      if (existsSync(localPath)) {
        imageBuffer = readFileSync(localPath);
      } else {
        process.stdout.write(`  [${i+1}/${subset.length}] Downloading ${img.post_id}... `);
  ...
  console.log(`Summary: ${success} succeeded (${downloaded} newly downloaded), ${failed} failed`);
}
...
async function main() {
  ...
  const redditPackPath = '/home/warby/Workspace/jupyter/packs/reddit-nightly-2026-01-03';
