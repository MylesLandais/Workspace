...
import textwrap
import types
from typing import Annotated, Any, Concatenate, TypeAlias, get_args, get_origin, get_type_hints
...
This is **extremely important** as wrong data cannot be detected! When asked for time data, do not specify the timezone. Return just the value, not json.
"""
...
def generate_camel_system_prompt(
    functions: Sequence[function_types.Function],
...
def _parse_args(
    ...
    field_fn: Callable[..., Any] = pydantic.Field,
    create_model_fn: Callable[..., type[BM]] = pydantic.create_model,
) -> type[BM]:
